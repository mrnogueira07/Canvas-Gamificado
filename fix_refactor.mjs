                ...
                <div className="flex items-center justify-between pb-3 mb-6 border-b border-violet-100">
                    <div>
                        <p className="text-[11px] font-black text-violet-700 uppercase tracking-widest">
                            {puzzles.length > 1 ? 'Múltiplos Quebra-Cabeças' : 'Quebra-Cabeça'} ({puzzles.length}/3)
                        </p>
                    </div>
                    {puzzles.length < 3 && (
                        <button
...
const startIndex = content.indexOf("const EXAMPLE_PUZZLE_URL");
...
content = content.substring(0, startIndex) + singlePuzzleCode + content.substring(endIndex);
fs.writeFileSync(path, content, 'utf8');
console.log("Successfully refactored DocumentView.tsx");
