...
def refactor():
    with open('src/components/generator/DocumentView.tsx', 'r', encoding='utf-8') as f:
        content = f.read()

    # We need to extract the interactive preview and upload into SinglePuzzle
    # and replace PuzzleSection body.
    
    # 1. We redefine PuzzleSection and introduce SinglePuzzle
    body = '''const EXAMPLE_PUZZLE_URL = 'https://images.unsplash.com/photo-1606326608606-aa0b62935f2b?w=600&q=80';
...
const SinglePuzzle: React.FC<{
    ...
    return (
        <div className="relative group/pcontainer p-6 mb-6 border-2 border-dashed border-violet-100 rounded-3xl bg-white shadow-sm hover:shadow-md transition-all">
            {canRemove && (
                <button
            ...
            <div className="flex items-center justify-between mb-4">
                <p className="text-[12px] font-black text-violet-700 uppercase tracking-widest">Quebra-Cabeça {pIdx + 1}</p>
                <select 
                    value={puzzle.pieceCount || 8}
                    onChange={(e) => onUpdatePuzzle(pIdx, { pieceCount: Number(e.target.value) })}
                    className="bg-slate-50 border border-slate-200 text-slate-700 text-[10px] uppercase font-black tracking-widest rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-violet-400"
                >
            ...
            <div className="space-y-4">
                ...
                >
                    ...
                    <div className="text-left">
                        <p className="text-[12px] font-black text-violet-700">Substituir Imagem do Puzzle {pIdx + 1}</p>
                        <p className="text-[9px] text-slate-400 font-medium mt-1">Clique para carregar foto do dispositivo (Máx 10MB)</p>
                    </div>
...
const PuzzleSection: React.FC<{
    ...
    const puzzles = gameLogic.puzzles || [{
        ...
        description: gameLogic.puzzleDescription || "Monte o quebra-cabeça e responda às questões!",
    ...
    const handleRemovePuzzle = (idx: number) => {
        ...
        const newPuzzles = puzzles.filter((_, i) => i !== idx);
        onEdit('puzzles', newPuzzles);
    };
    ...
    return (
        <div className="space-y-10">
            <div>
                <div className="flex items-center justify-between pb-3 mb-6 border-b border-violet-100">
                    <p className="text-[11px] font-black text-violet-700 uppercase tracking-widest">
                        Imagens do Quebra-Cabeça ({puzzles.length})
                    </p>
                    <button
                    ...
                    >
                        <span className="text-lg leading-none">+</span> Adicionar Puzzle
                    </button>
                </div>
            ...
            <div className="bg-white border border-violet-100 rounded-2xl p-4 shadow-sm mb-8">
                <Field
                    label="DESCRIÇÃO / CONTEXTO GERAL"
                    value={gameLogic.puzzleDescription || ''}
                    onChange={(val) => onEdit('puzzleDescription', val)}
                />
            </div>
                    ...
                    <div className="flex items-center gap-4">
                        {targets.length < 5 && (
                            <button
                                ...
                                className="text-[9px] flex items-center gap-1 font-black uppercase tracking-widest bg-emerald-100 text-emerald-600 hover:bg-emerald-200 px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
                            >
                            ...
                            <div key={i} className={group/pz relative p-6 rounded-[2rem] border transition-all  hover:shadow-xl hover:bg-white}>
                                ...
                                <div className="space-y-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <Field label="Fragmento / Nome do Assunto" value={toStr(target.title)} onChange={(val: string) => {
                                            if (!targets[i]) onEdit(	argets[], { ...target, title: val });
                                            else onEdit(	argets[].title, val);
                                        }} />
                                                ...
                                                <div key={oi} className={elative p-1 rounded-2xl border-2 transition-all }>
                                                    <Field
                                                        ...
                                                        onChange={(val: string) => {
                                                            ...
                                                            if (currentTarget.answer === opt) onEdit(	argets[].answer, val);
                                                        }}
                                                    />
                                                    <button
                                                        onClick={() => {
    ...
    new_content = re.sub(
        r"const EXAMPLE_PUZZLE_URL = 'https://images\.unsplash\.com/.*?};",
        body.replace('$', 'c:\Users\mathe\Documents\Game Canvas'),
        content,
        flags=re.DOTALL
    )

    with open('src/components/generator/DocumentView.tsx', 'w', encoding='utf-8') as f:
        f.write(new_content)

    print("Success replacing DocumentView")
