...
function refactor() {
    ...
    const singlePuzzleCode = `const EXAMPLE_PUZZLE_URL = 'https://images.unsplash.com/photo-1606326608606-aa0b62935f2b?w=600&q=80';
        ...
        <div className="relative group/pcontainer p-6 mb-8 border-2 border-dashed border-violet-100 rounded-3xl bg-white shadow-sm hover:shadow-md transition-all">
            ...
            <div className="flex items-center justify-between pb-3 mb-4 border-b border-violet-100">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-[11px] font-black text-violet-700 uppercase tracking-widest">Quebra-Cabeça {pIdx + 1}</p>
            ...
            <div className="space-y-4">
                    ...
                    >
                        {Array.from({ length: TOTAL }).map((_, idx) => {
                            ...
                            return (
                                ...
                                >
                                    <div className={\`absolute inset-0 border border-white/20 z-10 pointer-events-none \${placed ? 'opacity-0' : ''}\`} />
                                    {placed ? (
                ...
                >
                    ...
                    <div className="text-left">
                        <p className="text-[12px] font-black text-violet-700">Substituir Imagem (Quebra-Cabeça {pIdx + 1})</p>
                ...
                {/* Descrição */}
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 shadow-sm mt-4">
                    <Field
                        label="DESCRIÇÃO DA IMAGEM / CONTEXTO"
                        value={puzzle.description || ''}
                        onChange={(val) => onUpdatePuzzle(pIdx, { description: val })}
                    />
                </div>
            </div>
        </div>
                ...
                <div className="flex items-center justify-between pb-3 mb-6 border-b border-violet-100">
                    <div>
                        <p className="text-[11px] font-black text-violet-700 uppercase tracking-widest">
                            Imagens do Quebra-Cabeça ({puzzles.length}/3)
                        </p>
                    </div>
                    {puzzles.length < 3 && (
                        <button
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-3 max-w-2xl">
                    {targets.map((target, i) => {
                        ...
                        return (
                            <div key={i} className={\`group/pz relative p-6 rounded-[2rem] border transition-all \${target.isCorrect ? 'bg-indigo-50/30 border-indigo-100 shadow-sm' : 'bg-rose-50/50 border-rose-100'} hover:shadow-xl hover:bg-white\`}>
                                {targets.length > 3 && (
                                    <button
                                ...
                                <div className="flex items-center justify-between mb-6">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className={\`text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full border \${target.isCorrect ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : 'bg-rose-50 border-rose-100 text-rose-600'}\`}>
                                ...
                                <div className="space-y-6">
                                    <div className="grid grid-cols-1 gap-6">
                                        <Field label="Pergunta Desafio" value={toStr(target.question || target.title)} onChange={(val: string) => {
                                            if (!targets[i]) onEdit(\`targets[\${i}]\`, { ...target, question: val });
                                    ...
                                    <div className="pt-4 border-t border-slate-100">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            {(Array.isArray(target.options) ? target.options : ['Opção A', 'Opção B', 'Opção C']).slice(0, 3).map((opt: string, oi: number) => (
                                                <div key={oi} className={\`relative p-1 rounded-2xl border-2 transition-all \${opt === target.answer ? 'border-emerald-400 bg-emerald-50/30' : 'border-slate-50'}\`}>
                                                    <Field
                                                        ...
                                                        onChange={(val: string) => {
                                                            ...
                                                            newOpts[oi] = val;
                                                            if (!targets[i]) onEdit(\`targets[\${i}]\`, { ...currentTarget, options: newOpts, answer: wasCorrect ? val : currentTarget.answer });
                                                            else {
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            if (!targets[i]) onEdit(\`targets[\${i}]\`, { ...target, answer: opt });
                                                            else onEdit(\`targets[\${i}].answer\`, opt);
                                                        }}
                                                        className={\`absolute -top-2 -right-2 w-6 h-6 rounded-full flex items-center justify-center shadow-lg transition-all \${opt === target.answer ? 'bg-emerald-500 text-white scale-110' : 'bg-white text-slate-300 hover:text-emerald-500 opacity-0 group-hover/pz:opacity-100'}\`}
                                                    >
                                    ...
                                    <div className="pt-4 border-t border-slate-100">
                                        <Field label="Feedback Educativo (Ao acertar)" value={toStr(target.description || target.feedback || '')} onChange={(val: string) => {
                                            if (!targets[i]) onEdit(\`targets[\${i}]\`, { ...target, description: val });
