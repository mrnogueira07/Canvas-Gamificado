 ...
 * Responsável por capturar os parâmetros do plano de aula, upload de PDF e 
 * acionar a geração via IA (Gemini).
 */
...
export const GeneratorSidebar: React.FC<GeneratorSidebarProps> = ({
  ...
  const currentSubjects = useMemo(
    () => subjectsByLevel[formData.gradeLevel] || [],
    [formData.gradeLevel],
  );
  const currentYears = useMemo(
    () => yearsByLevel[formData.gradeLevel] || [],
    [formData.gradeLevel],
  );
  ...
  const handleSuggestContext = async () => {
    ...
    // Se os parâmetros básicos não estiverem preenchidos, avisar ou usar um fallback
    ...
    try {
      ...
      const prompt = `Como um Especialista em Game Design Educacional, sugira uma "IDEIA ou CONTEXTO" ÉPICO para um jogo. 
            Misture o conteúdo escolar com um tema que seja tendência na cultura digital (ex: Estilo Minecraft, Narrativa de RPG de fantasia, Estética Cyberpunk, Missão de Sobrevivência, Investigação Estilo Escape Room, ou Simulação de Vida em Marte). 
            
            Parâmetros:
            - Nível: ${formData.gradeLevel}
            - Série: ${formData.year}
            - Matéria: ${formData.subject}
            - Período: ${formData.quarter}
            
            A sugestão deve ser uma narrativa curta (máximo 180 caracteres) que transforme o assunto em uma jornada viciante.
            Retorne APENAS o texto da ideia em português, sem aspas ou introduções.`;
      ...
      if (text) {
        handleChange("additionalContext", text.trim());
      }
    } catch (error) {
      ...
      handleChange(
        ...
        formData.subject +
        " através de uma narrativa de exploração.",
      );
    } finally {
  ...
  const processFile = (file: File) => {
    ...
    if (file.size > 15 * 1024 * 1024) {
      setPdfError("O PDF deve ter no máximo 15 MB.");
      return;
    }
  ...
  if (!hasPdf && !hasContext) missingFields.push("Ideia ou Contexto");
      ...
      <div className="p-8 border-b border-slate-100">
        ...
        {viewOnly && (
          <div className="mt-4 px-3 py-2 bg-amber-500/10 border border-amber-500/20 rounded-xl">
            <p className="text-[10px] text-amber-600 font-black uppercase tracking-wider text-center">
              Modo Visualização
            </p>
          </div>
        )}
      ...
      <div className="p-8 space-y-8 flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
            ...
            <div className={inputWrapperClasses}>
              ...
              >
                <option value="" disabled className="bg-slate-100">
                  {formData.gradeLevel
                    ? "Selecione..."
                    : "Escolha o nível primeiro"}
                </option>
                {currentSubjects.map((s) => (
                  <option key={s} className="bg-slate-100">
                    {s}
                  </option>
                ))}
              </select>
          ...
          <div className="space-y-3">
            ...
            <div className={inputWrapperClasses}>
              ...
              >
                ...
                {currentYears.map((y) => (
                  <option key={y} className="bg-slate-100">
                    {y}
                  </option>
                ))}
              </select>
        ...
        <div className="space-y-3">
          ...
          <div className={inputWrapperClasses}>
            ...
            >
              ...
              {GAME_TYPES.map((g) => (
                <option key={g} className="bg-slate-100">
                  {g}
                </option>
              ))}
            </select>
        ...
        {!viewOnly && (
          <div className="space-y-3">
              ...
              {pdfFileName ? (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-black text-indigo-700 truncate tracking-tight">
                      {pdfFileName}
                    </p>
                    {isExtractingBncc ? (
            ...
            {pdfError && (
              <p className="text-[10px] text-rose-500 font-black text-center mt-2 px-4 leading-tight">
                {pdfError}
              </p>
            )}
            <input
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between ml-1">
              ...
              <button
                type="button"
                onClick={() =>
                  !isGenerating && !isSuggesting && handleSuggestContext()
                }
                className={`flex items-center gap-1.5 text-[9px] font-black transition-all uppercase tracking-widest ${isSuggesting ? "text-indigo-400" : "text-indigo-500 hover:text-indigo-600 active:scale-95 cursor-pointer"}`}
              >
                {isSuggesting ? (
                  <>
            ...
            <div
              className={`${inputWrapperClasses} rounded-[2rem] border-2 border-indigo-100 bg-white/50 focus-within:border-indigo-500/50 transition-all`}
            >
              <textarea
                className={`${innerInputClasses} h-36 resize-none p-5 leading-relaxed font-medium bg-transparent`}
                placeholder={
                  pdfFileName
                    ? "Adicione instruções extras para o seu PDF..."
                    : "Descreva sua ideia ou clique em sugerir para inspiração (ex: Ensinar frações usando música)..."
                }
                value={formData.additionalContext}
                disabled={viewOnly || isGenerating || isSuggesting}
                onChange={(e) =>
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) {
                    e.preventDefault();
                    if (isFormReady && !isGenerating) onGenerate();
                  }
                }}
              />
            </div>
